...
_How we show this to a real end user and earn pull, not push. Justin's positioning +
Nicole's activation, made concrete. The honest stance: **we do not have PMF yet — we have
a credible wedge and a testable riskiest assumption.**_

---

## 1. Position narrow (Justin)
...
- A single cohort is a **dense, closed graph** — the ideal shape for a viral loop. Density
  beats reach at the start.

Expansion only **after** the loop proves out in one cohort: copy to the next Stellar
cohort, then adjacent dev communities. Horizontal "everyone" is a post-PMF question.

## 3. The riskiest assumption (test before anything else)
...
If a minted vouch's claim-completion is below 40%, the mechanic is wrong — fix friction or
the reward feeling before scaling. (This is cheap, fast, and decisive.)

## 4. The end-user experience — how they meet us
...
Distribution is not B2C (we don't acquire users one by one). It's **B2Community2C**: a
**cohort leader** (hackathon organizer / bootcamp instructor / DAO lead) seeds it —

1. Leader "vouches their cohort" or kicks off a vouch round at an event.
2. Each member claims, then vouches 2–3 others → the half-card link does the spreading.
3. The cohort's constellation becomes a shared artifact (and a flex).
4. Saturate one cohort → replicate the playbook to the next.

The product's job is to make the **leader look good** and the **share moment irresistible**
(great OG cards, instant claim). The leader is the channel.
...
## 8. Why it can win (and where it's weak)

- **Strengths:** novel on Stellar (no consumer social-reputation game exists), a viral
  install loop baked into the mechanic, sub-cent fees make every micro-action affordable,
  honest anti-sybil that survives scrutiny.
- **Weak spot (own it):** traction. We have mechanics, not yet users. The whole GTM is
  engineered to manufacture the one thing we lack — a dense, real cohort that completes
  the loop.
